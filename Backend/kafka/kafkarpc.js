//makes req to broker ,initializae consumer ,get res from broker ,matches which response I got
...
KafkaRPC.prototype.makeRequest = function(topic_name, content, callback) {
  //makes corr id for we need to map res and req
  //we can use uuid in correlation id
  self = this;
  ...
  var entry = {
    callback: callback, //callback for every req we make
    timeout: tId //the id for the timeout so we can clear it   //each req has this id
  };
  ...
  self.setupResponseQueue(self.producer, topic_name, function() {
    ...
    console.log("in response1:before response is sent by producer");
    console.log(self.producer.ready);
    self.producer.send(payloads, function(err, data) {
      console.log("in response2:producer sending response");
...
KafkaRPC.prototype.setupResponseQueue = function(producer, topic_name, next) {
  ...
  consumer.on("message", function(message) {
    ...
    if (correlationId in self.requests) {
      ...
      //delete the entry from hash
      delete self.requests[correlationId]; //from memory
      //callback, no err
